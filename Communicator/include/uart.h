...
class UART : public COMMUNICATOR {
 public:
  ...
  ~UART() override;
  int receive(unsigned char *buffer) override;
  int send(double X, double Y, double Z, double center_Z) override;
  int senderr() override { return 0; }
 ...
 private:
  ...
  int uart_write(const unsigned char *w_buf, size_t len);
  int uart_read(unsigned char *r_buf, size_t len);
  UART() : fd(-1) {}
};
