...
int UART::uart_read(unsigned char *r_buf, size_t len) {
  ssize_t cnt = 0;
  fd_set rfds;
  struct timeval time;
...
int UART::uart_write(const unsigned char *w_buf, size_t len) {
  ssize_t cnt = 0;
...
int UART::send(double X, double Y, double Z, double center_Z) {
  /* configure can_id and can data length */
 // printf("Let us send a number!\n");
  unsigned char frame[11];
  short x100 = X * 1000;
  short y100 = Y * 1000;
  short z100 = Z * 1000;
  short center_z = center_Z * 1000;
  ...
  frame[9] = (unsigned char)(center_z >> 8);
  frame[10] = (unsigned char)(center_z);
...
int UART::receive(unsigned char *buffer) {
